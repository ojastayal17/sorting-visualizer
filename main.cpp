...
SliderSFML::SliderSFML(int x, int y)
{
    ...
    if (!font.loadFromFile("Vremyafwf-Rp1e.ttf"))
...
{
    logic(window);
    window.draw(returnText(xCord +16, yCord -55, "Speed Delay", 20));
    ...
    window.draw(slider);
    window.draw(returnText(slider.getPosition().x - sliderWidth+13, slider.getPosition().y - sliderHeight,
...
void BubbleSort(vector<rectangle> &rectangles, RenderWindow &window, Text process,SliderSFML slider)
{
...
void mergeall(vector<rectangle> &rectangles, int starti, int endi, RenderWindow &window, Text process,SliderSFML &slider)
{
    vector<rectangle> output;

    output.resize((endi-starti)+1);
    ...
    while(i <= mid && j <= endi)
    {
        ...
        {
            output[k].value = rectangles[i].value;
            output[k].rect.setPosition(startX+i*rectStart,startY);
            output[k].rect.setSize(rectangles[i].rect.getSize());
            window.clear();
            window.draw(process);
            slider.draw(window);
            speed = slider.getSliderValue();
        ...
        {
            output[k].value = rectangles[j].value;
            output[k].rect.setPosition(startX+j*rectStart,startY);
            output[k].rect.setSize(rectangles[j].rect.getSize());
            window.clear();
            window.draw(process);
            slider.draw(window);
            speed = slider.getSliderValue();
    ...
    {
        rectangles[l].value = output[x].value;
        rectangles[l].rect.setPosition(output[x].rect.getPosition());
        rectangles[l].rect.setSize(output[x].rect.getSize());
        window.clear();
        window.draw(process);
        slider.draw(window);
        speed = slider.getSliderValue();
        drawStates(&window,rectangles,-1,l);
        std::this_thread::sleep_for(std::chrono::milliseconds((int)speed));//delay
        window.display();
        x++;
    }
...
void MergeSort(vector<rectangle> &rectangles,int starti, int endi,RenderWindow &window, Text process,SliderSFML &slider)
{
    if(starti>=endi)
    {
        return;
    ...
    MergeSort(rectangles,starti,mid,window,process,slider);
    MergeSort(rectangles,mid+1,endi,window,process,slider);

    mergeall(rectangles,starti,endi,window,process,slider);
}

void RadixSort(vector<rectangle> &rectangles, RenderWindow &window, Text process,SliderSFML slider)
{
...
void CocktailSort(vector<rectangle> &rectangles, RenderWindow &window, Text process,SliderSFML slider)
{
    bool swapped = true;
    int sm,start = 0;
    sm=start;
...
int main()
{
    ...
    for(int i=0; i<n+2; i++)
    {
        ...
        case 0:
        {
            ...
            item.setPosition(10,350);
            break;
        }
        case 1:
        {
            item.setString("Enter 2 for Insertion Sort");
            item.setPosition(10,400);
            break;
        }
        case 2:
        {
            item.setString("Enter 3 for Bubble Sort");
            item.setPosition(10,450);
            break;
        }
        case 3:
        {
            item.setString("Enter 4 for Radix Sort");
            item.setPosition(10,500);
            break;
        }
        case 4:
        {
            item.setString("Enter 5 for Merge Sort");
            item.setPosition(10,550);
            break;
        }
        case 5:
        {
            item.setString("Enter 6 for Cocktail Sort");
            item.setPosition(10,600);
            break;
        }
        case 6:
        {
            item.setString("Sorting Algorithms");
            item.setPosition(10,300);
            break;
        }
        case 7:
        {
            item.setString("Time (in Seconds)");
    ...
    t.setPosition(10,550);

    Text process;
    process.setFont(font);
    process.setCharacterSize(40);
    process.setString("Processing....");
    process.setPosition(10,600);
    ...
    for(int i=0; i<n; i++)
    {
        ...
        case 0:
        {
            sortTime.setPosition(500,350);
            break;
        }
        case 1:
        {
            sortTime.setPosition(500,400);
            break;
        }
        case 2:
        {
            sortTime.setPosition(500,450);
            break;
        }
        case 3:
        {
            sortTime.setPosition(500,500);
            break;
        }
        case 4:
        {
            sortTime.setPosition(500,550);
            break;
        }
        case 5:
        {
            sortTime.setPosition(500,600);
            break;
        }
        }
    ...
    backMenu.setCharacterSize(35);
    backMenu.setString("press 0 to back to menu");
    backMenu.setPosition(10,450);
    ...
    while (window.isOpen())
    {
        ...
        else if(sorting && type == 3) // Bubble Sort
        {
            if(!sorted)
            {
                ...
                BubbleSort(rectangles,window,process,slider);
        ...
        else if(sorting && type == 4)//Radix Sort
        {
            if(!sorted)
            {
                clock_t start = clock();

                RadixSort(rectangles,window,process,slider);
        ...
        else if(sorting && type == 5) // merge sort
        {
            ...
            if(!sorted)
            {
                ...
                MergeSort(rectangles,0,rectangles.size()-1,window,process,slider);
                mergeall(rectangles,0,rectangles.size()-1,window,process,slider);
        ...
        else if(sorting && type == 6) //Cocktail Sort
        {
            if(!sorted)
            {
                ...
                CocktailSort(rectangles,window,process,slider);
